...
{
	/// <summary>
	/// This Command encapsulates a Task that has no return. Unlike other commands, a VoidTaskCommand can only 
	/// be executed once, because Tasks can only be run once. This command ignores abort requests, and the runtime
	/// arg value that can be passed as an argument to the execution methods is ignored.
	/// </summary>
	public class VoidTaskCommand : AsyncCommand
	{
		/// <summary>
		/// Constructor
		/// </summary>
		...
		/// </param>
		public VoidTaskCommand(Task task) : this(task, null)
		{
		}
		...
		/// </param>
		public VoidTaskCommand(Task task, Command owner) : base(owner)
		{
			this.task = task;
		}
		...
		protected sealed override void AsyncExecuteImpl(ICommandListener listener, object runtimeArg)
		{
			...
			task.ContinueWith(t =>
			{
				...
				if (Thread.CurrentThread.ManagedThreadId == startingThreadId)
				{
					...
					try
					{
						thread.Start(new AsyncThreadArg(listener, null));
					}
		...
		private void ExecuteAsyncRoutine(Object arg)
		{
			...
			{
				threadArg.Listener.CommandSucceeded(null);
			}
			else
			{
